...
public class DatabaseConnectionCache {
	...
	public DatabaseConnectionCache(Context context, String table,
			HashMap<String, String> columns) {
		this.db_table = table;
		this.context = context;
		this.db_create = "create table " + table
				+ " ( ID integer primary key autoincrement,";
		Iterator<String> keys = columns.keySet().iterator();
		while (keys.hasNext()) {
			String current_key = keys.next();
			this.db_create += ", " + current_key + " "
					+ columns.get(current_key);
		}
		this.db_create += " );";
	}
	...
	public Cursor fetchAll() {
		return db.query(db_table, (String[]) columns.toArray(), null, null,
				null, null, null);
	}

	public Cursor fetchRow(long rowId) {
		Cursor cursor = db.query(true, db_table, (String[]) columns.toArray(),
